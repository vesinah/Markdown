...
ชุดข้อมูล DISTINGUO ได้รับการสร้างสรรค์โดยอ้างอิงรหัสจำแนกพื้นที่ (Zone Types) ตามหลักการควบคุมสัญญะของ SegmOnto อย่างเคร่งครัด โดยสะท้อนผ่านทางโครงสร้างหน้าเอกสาร XML PAGE
  ...
  <Page imageFilename="distinguo_sermon_folio_45r.jpg" imageWidth="2200" imageHeight="3000">
    ...
    <TextRegion id="reg_01" type="CustomZone#MainText">
      ...
      <TextLine id="l_01">
        ...
        <TextEquiv>
          <Unicode>In principio erat Verbum et Verbum erat apud Deum...</Unicode>
        </TextEquiv>
      </TextLine>
    </TextRegion>
...
sequenceDiagram
    ...
    XMLParser->>XMLParser: อ่านแท็กชนิด Zone (type=CustomZone#...) และดึงพิกัดพิกเซลโพลีกอน
...
class SegmontoMaskGenerator:
    ...
    def parse_xml_and_draw_masks(self, xml_path, original_image_shape):
        ...
        for region in root.findall('.//page:TextRegion', self.namespaces):
            ...
            # แปลงข้อความจุดพิกัด "x1,y1 x2,y2 ..." เป็นลิสต์ของพิกัดตัวเลข
            pts = []
