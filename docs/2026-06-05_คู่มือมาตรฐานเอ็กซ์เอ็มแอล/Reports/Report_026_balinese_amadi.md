...
| **ลิงก์เข้าถึงระบบ (URL)** | [huggingface.co/datasets/balinese-lontar-amadi](https://huggingface.co/datasets/balinese-lontar-amadi) (หน้าฐานข้อมูลระดับบรรทัดและคลังน้ำหนักปัญญาประดิษฐ์เพื่อการอนุรักษ์) |
...
class BalineseHTR_CNNRNN(nn.Module):
    """
    แบบจำลองโมเดลตรวจวิเคราะห์และรู้จำตัวอักษรจารึกบาหลีระดับบรรทัดโบราณ
    สถาปัตยกรรม CNN-BiGRU-CTC ปรับแต่งพิเศษสัดส่วนน้ำหนักกะทัดรัด (Edge Device Optimized)
    """
...
if __name__ == "__main__":
    ...
    mock_dir = "D:/01_APP/Research/scratch"
    os.makedirs(mock_dir, exist_ok=True)
    
    raw_mock_image_path = os.path.join(mock_dir, "amadi_lontar_folio_204_l3.png")
    ...
    cv2.imwrite(raw_mock_image_path, mock_raw_image)
    print(f"[สำเร็จ] จัดเตรียมภาพใบลานดิบจำลองพร้อมเสี้ยนไม้รบกวนที่: {raw_mock_image_path}")
    
    # 2. รันอัลกอริทึม Bilateral + Sauvola Adaptive Thresholding
    try:
        restored_img, saved_path = restore_balinese_palm_leaf_grain(raw_mock_image_path, output_dir=mock_dir)
