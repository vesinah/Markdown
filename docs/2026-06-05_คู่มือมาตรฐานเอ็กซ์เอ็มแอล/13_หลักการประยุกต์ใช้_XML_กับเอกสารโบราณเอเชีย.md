...
PAGE XML ถูกออกแบบมาเพื่อความยืดหยุ่นที่สูงกว่าแต่แรก ทำให้เหมาะกับงานวิจัยเฉพาะทาง ดังที่ **F.X. Erhard (2025)** ได้แสดงให้เห็นใน *"Text and Layout Recognition for Tibetan Newspapers with Transkribus"* ว่า PAGE XML มีบทบาทสำคัญในการแยกแยะองค์ประกอบที่ซับซ้อนในหน้ากระดาษ
...
โครงสร้าง `<OrderedGroup>`, `<UnorderedGroup>`, และ `<Relations>` จึงเป็นสิ่งจำเป็น
...
สำหรับ ALTO การจัดการสิ่งเหล่านี้จะพึ่งพาการนิยาม `<StructureTag>` ล่วงหน้า จากนั้นบรรทัดต่างๆ จะดึงแท็กเหล่านั้นมาใช้ผ่าน `TAGREFS` สำหรับทิศทาง จะระบุผ่าน `BASEDIRECTION`
