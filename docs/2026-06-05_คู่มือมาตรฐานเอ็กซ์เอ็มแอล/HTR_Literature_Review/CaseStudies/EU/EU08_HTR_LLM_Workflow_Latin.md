...
| **สถาบัน (Institutions)** | [ไม่มีข้อมูลระบุในแหล่งอ้างอิงเบื้องต้น] |
...
### 1.1 ภาพรวมโปรเจกต์
เป้าหมายของงานวิจัยชิ้นนี้คือการแก้ไขปัญหาการถอดความ (Transcription) เอกสารทางกฎหมายยุคกลางภาษาละติน (Abbreviated Latin Court Hand) ซึ่งขึ้นชื่อเรื่องการใช้คำย่ออย่างหนักและการเขียนตัวหวัด ผู้วิจัยได้นำเสนอ Workflow แบบลูกผสม (Hybrid Pipeline) ที่ใช้ทั้ง Specialized HTR Model ดั้งเดิม และ Large Language Models (LLMs) เข้ามาทำงานร่วมกันถึง 4 ขั้นตอน เพื่อให้ได้ผลลัพธ์ระดับความแม่นยำสูง (High-Accuracy) เทียบเท่านักวิชาการ 

### 1.2 ลักษณะเอกสาร
- เป็นเอกสาร Court Hand ซึ่งเป็นลายมือทางกฎหมายของยุโรปยุคกลาง
- **ความท้าทายหลัก:** การใช้สัญลักษณ์ย่อคำ (Abbreviations) ที่ซับซ้อน ซึ่งถ้าหากทำ HTR ปกติจะได้ผลลัพธ์เป็นข้อความที่มีคำย่ออ่านไม่รู้เรื่อง จำเป็นต้องให้ผู้เชี่ยวชาญกระจายคำย่อ (Expansion) ออกมาเป็นคำละตินเต็มรูปแบบ

### 1.3 บริบทสำคัญ (Background)
แนวทางดั้งเดิมมักใช้ HTR สกัดข้อความดิบ และต้องใช้มนุษย์แปลคำย่อ แต่งานวิจัยนี้ผลักดันให้ LLMs รับหน้าที่ Post-correction และ Expansion แทนมนุษย์
...
- การใช้ตัวย่อที่รุนแรงและสัญลักษณ์เฉพาะทางกฎหมาย (Court Hand) ที่ไม่มีใช้ในภาษาเขียนปกติ

### 5.2 ความท้าทายด้านเทคนิค
- การทำ OCR เพียงอย่างเดียวไม่สามารถให้ผลลัพธ์ที่นำไปใช้วิเคราะห์ต่อได้ เพราะคำย่อไม่ได้แปลความหมายในตัวมันเอง ต้องมีการ Expansion ก่อนเสมอ

### 5.4 บทเรียนสำคัญ
- การนำรูปภาพต้นฉบับส่งเข้าไปใน LLM (Multimodal Post-Correction) พร้อมกับ Baseline Transcription จาก HTR ทำให้ LLM สามารถตรวจสอบ (Cross-check) ด้วยตาตัวเองได้ ซึ่งลดอัตราการ "หลอน (Hallucination)" ของ LLM ในขั้นตอนการแก้คำผิดได้ดีเยี่ยม
...
*(ยังไม่มีข้อมูล URL ของ Dataset หรือ Repository ใน Source ปัจจุบัน)*
...
1. Joshua D. Isom, *"An HTR-LLM Workflow for High-Accuracy Transcription and Analysis of Abbreviated Latin Court Hand,"* arXiv preprint, 2025. [arXiv:2507.04132](https://arxiv.org/abs/2507.04132)
