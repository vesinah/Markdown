...
11. [แหล่งอ้างอิง](#แหล่งอ้างอิง) [1]
...
การแบ่งพื้นที่ในแต่ละหน้า เป็นจุดที่แตกต่างกันอย่างมาก ALTO เน้นไปที่การตีกรอบหน้ากระดาษแบบคลาสสิก (มี Margin) ส่วน PAGE เน้นขอบเขตที่ยืดหยุ่นด้วยรูปหลายเหลี่ยม (Polygon) ซึ่งเหมาะกับเอกสารทางประวัติศาสตร์ที่มีรอยขาดหรือรูปทรงไม่แน่นอน [2]
...
ลำดับขั้นของข้อความ เป็นหัวใจหลักของการเก็บข้อมูล OCR / HTR ทั้งสองมาตรฐานมีลำดับขั้นที่คล้ายกัน แต่จุดที่ต่างกันคือการจัดการคำ (Word) และองค์ประกอบย่อยอื่นๆ [3]
...
การเก็บประวัติที่มาที่ไปของเอกสาร (Provenance) และซอฟต์แวร์ที่ใช้ เป็นสิ่งที่สำคัญต่อเอกสารวิชาการ เพื่อตรวจสอบย้อนกลับ (Traceability) [4]
...
### แผ่นสคริปต์ XSLT สำหรับแปลงรูปแบบไฟล์
```xml
<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet version="1.0" 
    ...
    <xsl:template match="/alto:alto">
        <page:PcGts xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
            ...
            <page:Metadata>
                <page:Creator>Unified OCR/HTR Converter Engine v1.0</page:Creator>
                <page:Created>2026-06-05T11:27:30</page:Created>
                <page:LastChange>2026-06-05T11:27:30</page:LastChange>
                <page:Comments>Auto-converted from ALTO XML</page:Comments>
            </page:Metadata>
    ...
    <xsl:template match="alto:Page">
        <page:Page imageFilename="{alto:sourceImageInformation/alto:fileName}" 
            imageWidth="{@WIDTH}" imageHeight="{@HEIGHT}">
            <page:PrintSpace>
                <page:Coords points="0,0 {@WIDTH},0 {@WIDTH},{@HEIGHT} 0,{@HEIGHT}"/>
                <xsl:for-each select="alto:PrintSpace/alto:TextBlock">
                    <page:TextRegion id="{@ID}" type="paragraph">
                        <page:Coords points="{@HPOS},{@VPOS} {@HPOS+@WIDTH},{@VPOS} {@HPOS+@WIDTH},{@VPOS+@HEIGHT} {@HPOS},{@VPOS+@HEIGHT}"/>
                        <xsl:for-each select="alto:TextLine">
                            <page:TextLine id="{@ID}">
                                <page:Coords points="{@HPOS},{@VPOS} {@HPOS+@WIDTH},{@VPOS} {@HPOS+@WIDTH},{@VPOS+@HEIGHT} {@HPOS},{@VPOS+@HEIGHT}"/>
                                <page:Baseline points="{@HPOS},{@VPOS+@HEIGHT} {@HPOS+@WIDTH},{@VPOS+@HEIGHT}"/>
                                <page:TextEquiv>
                                    <page:Unicode>
                                        <xsl:for-each select="*">
                                            <xsl:choose>
...
## ภาคผนวก ง: คู่มือปฏิบัติการ: การแปลงโครงสร้างทางกายภาพและเรขาคณิตระหว่าง ALTO XML และ PAGE XML
ในการเชื่อมโยงเครื่องมือรู้จำอักขระรุ่นต่างๆ และการสร้างคลังเอกสารดิจิทัลที่มีความเข้ากันได้ การแปลงรูปแบบข้อมูล (Format Conversion) ระหว่างมาตรฐาน ALTO XML และ PAGE XML จึงเป็นสิ่งจำเป็น ความท้าทายหลักอยู่ที่การแปลงพิกัดจุด (Polygonal coordinates) ใน PAGE XML ให้กลับไปเป็นกรอบสี่เหลี่ยม (Bounding Box) ใน ALTO XML หรือในทางกลับกันการประมาณค่าพิกัดจุดจากกรอบสี่เหลี่ยม

### ตารางสรุปการแปลงคุณลักษณะที่สำคัญ

| โครงสร้างระดับ | มาตรฐาน PAGE XML | มาตรฐาน ALTO XML | แนวทางการแปลงข้อมูล |
| :--- | :--- | :--- | :--- |
| รากหลัก (Root) | `<PcGts>` | `<alto>` | ต้องทำการประกาศ Schema และ Namespace ให้ถูกต้องตามเวอร์ชัน |
| ภาพต้นฉบับ | `@imageFilename` | `<fileName>` ภายใต้ `<sourceImageInformation>` | ทำการคัดลอกค่าสตริงตรงๆ ระหว่างแอตทริบิวต์กับแท็กข้อมูล |
| บล็อกข้อความ | `<TextRegion>` | `<TextBlock>` | แมปรหัสไอดีและประเภท (เช่น paragraph -> TextBlock) |
| แนวบรรทัด | `<TextLine>` | `<TextLine>` | แมปพิกัดเรขาคณิตและคำนวณกล่องสี่เหลี่ยมล้อมรอบค่าพิกัดสูงสุด |
| ผลลัพธ์ตัวอักษร | `<Unicode>` ภายใต้ `<TextEquiv>` | `@CONTENT` ในแท็ก `<String>` | รวมกลุ่มค่าคำและประยุกต์ใช้ `<SP>` คั่นระหว่างประโยคหรือคำ |

การแปลงรูปแบบดังกล่าวสามารถทำงานได้อย่างรวดเร็วผ่านการเขียนสคริปต์ XSLT (Extensible Stylesheet Transformations) ร่วมกับเอนจินภาษา Python สำหรับระบบวิเคราะห์วิจัยอักษรไทยที่ต้องการขยายฐานข้อมูลในคลังเอกสารระดับชาติ

### เชิงอรรถ
[1] Library of Congress. *ALTO Schema Mapping and Migration Guides*. (Library of Congress, 2020).
[2] Clausner, Christian, Stefan Pletschacher, and Apostolos Antonacopoulos. "A Framework for the Preservation and Conversion of Document Layout Metadata." In *Proceedings of the 11th International Conference on Document Analysis and Recognition (ICDAR)*, 928-932. (IEEE, 2011). doi:10.1109/ICDAR.2011.189.
[3] Kay, Michael. *XSLT 2.0 and XPath 2.0 Programmer's Reference*. (Wrox, 2008).
[4] Tennant, Roy. "XML: The Lingua Franca of Digital Libraries." *Library Journal* 127, no. 11 (2002): 34-36.
