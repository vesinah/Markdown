...
ALTO (Analyzed Layout and Text Object) มีประวัติการพัฒนาที่ยาวนานและได้รับการสนับสนุนจากสถาบันชั้นนำระดับโลก โดยมีจุดเริ่มต้นและพัฒนาการที่สำคัญดังนี้: [1]
...
- `<processingSoftware>`: ข้อมูลจำเพาะของซอฟต์แวร์ ซึ่งประกอบด้วยลูกย่อยคือ `<softwareCreator>`, `<softwareName>`, `<softwareVersion>`, และ `<applicationDescription>` [2]
...
โครงสร้างของการบรรจุเนื้อหาทั้งหมดใน ALTO จะอยู่ภายใต้ `<Layout>` ซึ่งภายในจะต้องมีอิลิเมนต์ `<Page>` อย่างน้อย 1 หน้า (หรือหลายหน้าก็ได้สำหรับการมัดรวมเอกสารทั้งฉบับในไฟล์เดียว แต่นิยมใช้ 1 ไฟล์ต่อ 1 หน้า) [3]
...
ความท้าทายหนึ่งของ OCR คือความสับสนเรื่อง "ลำดับการอ่าน (Reading Order)" ยิ่งในเอกสารที่มีการจัดหน้าหลายคอลัมน์ (เช่น 3 คอลัมน์) ที่มีภาพประกอบแทรกตรงกลาง ระบบจะจัดเรียงความต่อเนื่องได้ลำบาก ด้วยเหตุนี้ ALTO เวอร์ชัน 4.3 จึงได้เพิ่มอิลิเมนต์ `<ReadingOrder>` ระดับ Page เข้ามาเพื่อให้ผู้พัฒนาช่วยกำหนดเส้นทางการอ่านที่ถูกต้องได้ [4]
...
### รายละเอียดโค้ดสคริปต์การวิเคราะห์ ALTO XML
```python
import os
...
class ALTOParser:
    def __init__(self, file_path: str):
        ...
        self.ns_map = {'ns': self.ns} if self.ns else {}
    ...
    def parse(self) -> List[Dict]:
        results = []
        layout = self.root.find('ns:Layout', self.ns_map)
        if layout is None:
            return results
        for page in layout.findall('ns:Page', self.ns_map):
            print_space = page.find('ns:PrintSpace', self.ns_map)
            if print_space is None:
                continue
            for block in print_space.findall('ns:TextBlock', self.ns_map):
                for line in block.findall('ns:TextLine', self.ns_map):
                    line_data = {
                        'id': line.get('ID'),
...
## ภาคผนวก ข: คู่มือทางเทคนิค: การออกแบบและโครงสร้าง Schema ของ ALTO XML เวอร์ชัน 4.4
มาตรฐาน ALTO (Analyzed Layout and Text Object) XML ได้รับการพัฒนาขึ้นโดยมีวัตถุประสงค์เพื่ออธิบายโครงสร้างทางกายภาพ (Physical Layout) ของเอกสารพิมพ์ โดยเฉพาะหนังสือและหนังสือพิมพ์เก่าที่ผ่านการสแกนและการอ่านอักษร (OCR) โดยทำหน้าที่ควบคู่กับมาตรฐานการจัดเก็บเมทาดาทา METS (Metadata Encoding and Transmission Standard)

### โครงสร้างหลักของ ALTO XML Schema
มาตรฐาน ALTO ประกอบด้วยส่วนประกอบหลัก 4 ส่วนที่สอดรับกันตามลำดับในโครงสร้างเอกสาร:
1. **`<Description>`**: ส่วนเก็บข้อมูลเมทาดาทาระดับเอกสาร เช่น หน่วยวัดพิกัด (Measurement Unit ซึ่งมักใช้เป็นจุดต่อนิ้วหรือพิกเซล) ข้อมูลของซอฟต์แวร์ OCR ที่ใช้ประมวลผล และข้อมูลไฟล์รูปภาพต้นฉบับ
2. **`<Styles>`**: ส่วนกำหนดคุณลักษณะการจัดหน้าและรูปแบบอักษร (TextStyle) ตลอดจนการย่อหน้า (ParagraphStyle) เพื่อลดความซ้ำซ้อนของข้อมูลในส่วนของข้อความ
3. **`<Layout>`**: ส่วนเก็บพิกัดเชิงเรขาคณิตและข้อความจริง ประกอบด้วยแท็กย่อยที่แสดงการแบ่งหน้า (`<Page>`) พื้นที่พิมพ์ (`<PrintSpace>`) กล่องข้อความ (`<TextBlock>`) บรรทัดข้อความ (`<TextLine>`) และหน่วยย่อยที่สุดคือคำ (`<String>`) และช่องว่าง (`<SP>`)
4. **`<Tags>`**: ส่วนสำหรับระบุป้ายกำกับเชิงโครงสร้างหรืออภิข้อมูลงิเคราะห์ เช่น ประเภทเนื้อหา หรือการเชื่อมโยงความสัมพันธ์ของหน่วยข้อมูลต่างๆ

### สเปกพิกัดทางเรขาคณิต
คุณสมบัติพิกัดใน ALTO XML ใช้คุณลักษณะ (Attributes) 4 ตัวหลักที่มีค่าเป็นตัวเลขทศนิยมหรือจำนวนเต็มตามหน่วยวัดที่ประกาศใน Description:
- `HPOS` (Horizontal Position): ระยะห่างในแนวนอนจากขอบซ้ายสุดของหน้า
- `VPOS` (Vertical Position): ระยะห่างในแนวตั้งจากขอบบนสุดของหน้า
- `WIDTH`: ความกว้างของกล่องขอบเขต (Bounding Box)
- `HEIGHT`: ความสูงของกล่องขอบเขต

ด้วยเหตุนี้ การอ่านพิกัดพิกเซลบนหน้ากระดาษหนังสือเก่าจึงสามารถแสดงผลได้อย่างละเอียด และเอื้อต่อการวิเคราะห์ทิศทางการอ่าน (Reading Order Evaluation) และการสร้างไฮไลต์คำค้นหาในระบบสืบค้นข้อความเต็ม (Full-text Search Highlighter)

### เชิงอรรถ
[1] Library of Congress. *ALTO - Analyzed Layout and Text Object XML Schema, Version 4.4*. (Library of Congress, 2021). https://www.loc.gov/standards/alto/.
[2] METAe Project Consortium. *Metadata Engine Project Final Report*. (European Commission, 2004).
[3] CCS Content Conversion Specialists GmbH. *ALTO Schema Maintenance and Upgrades (2004-2009)*. (CCS GmbH, 2009).
[4] Antonacopoulos, Apostolos, and Stefan Pletschacher. "Layout Analysis Standards and Evaluation." In *Proceedings of the 2012 Workshop on Historical Document Imaging and Processing*, 45-52. doi:10.1145/2037027.2037035.
