...
| **ลิงก์เข้าถึงระบบ (URL)** | [lannadigital.library.cmu.ac.th](https://lannadigital.library.cmu.ac.th/) (ฐานข้อมูลหลักสถาบันวิจัยสังคม มหาวิทยาลัยเชียงใหม่) | [1]
...
def parse_pagexml_lanna(xml_path):
    """
    พาร์สไฟล์ PageXML เพื่อสกัดตำแหน่งรูปภาพ ข้อมูล Baseline โพลีกอนครอบบรรทัด และข้อความเฉลยจารึกต้นฉบับธรรมล้านนา
    """
...
def crop_and_clean_line_by_baseline(image_path, line_data, output_folder):
    ...
    if len(boundary) < 3:
        ...
        # ถอดมิติความกว้างและสูงโดยเลี่ยงดัชนีดวงเล็บเดี่ยว
        img_h_shape, img_w_shape, _ = img.shape
        # ทำการ Padding ชดเชยตัวเชิงห้อยล่างและสระบนล้านนา
        y_start = max(0, y_min - 45)
        y_end = min(img_h_shape, y_min + h_box + 45)
        x_start = max(0, x_min - 15)
        x_end = min(img_w_shape, x_min + w_box + 15)
...
if __name__ == "__main__":
    ...
    base_dir = "d:/01_APP/HTR_Research/scratch"
    ...
    mock_img = np.zeros((200, 1000, 3), dtype=np.uint8)
    # กำหนดค่าสีเบจด้วยการกำหนดค่าแบบแถวแทนวงเล็บเหลี่ยมดัชนีเดี่ยวเพื่อความปลอดภัยทางอ้างอิง
    mock_img[:] = [160, 200, 230] # Blue, Green, Red (Light beige-brown leaf color)
...
รายงานฉบับนี้นำเสนอระบบการแปลงและจดจำลายมืออักษรธรรมล้านนาและอักษรไทยนิเทศบนเอกสารใบลานโบราณ โดยความร่วมมือระหว่างสถาบันวิจัยสังคม มหาวิทยาลัยเชียงใหม่ และแพลตฟอร์ม e-Scriptorium โครงการนี้ชูเทคโนโลยีการสร้างเส้นฐานแนวเขียน (Baseline Segmentation) และการจดจำข้อความระดับบรรทัดด้วยกลไก Kraken เพื่อลดขั้นตอนการสกัดพิกัดตัวอักษรเดี่ยวที่มักประสบความล้มเหลวในคัมภีร์ใบลานโบราณที่มีการซ้อนสระและพยัญชนะในแนวดิ่ง ส่งผลให้สามารถผลิตข้อความปริวรรตดิจิทัลเพื่อการสืบค้นได้อย่างมีประสิทธิภาพสูงสุด

---

### เชิงอรรถ

[1] สถาบันวิจัยสังคม มหาวิทยาลัยเชียงใหม่ (CMU-SRI) และกลุ่มวิจัย e-Scriptorium, "โครงการจัดทำระบบนิเวศข้อมูลและคลังเอกสารประวัติศาสตร์ล้านนาดิจิทัล," (เชียงใหม่: มหาวิทยาลัยเชียงใหม่, 2024), https://lannadigital.library.cmu.ac.th.
