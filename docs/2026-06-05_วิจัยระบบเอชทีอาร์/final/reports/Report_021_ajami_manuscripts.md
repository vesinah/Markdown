...
| **ลิงก์อ้างอิงวิชาการ (Publication URL)** | [link.springer.com/chapter/10.1007/978-3-032-04627-7_36](https://link.springer.com/chapter/10.1007/978-3-032-04627-7_36) | [1]
...
### 10.2 โค้ดต้นแบบ Python สำหรับการทำ Sauvola Binarization และการประยุกต์แปลงภาพเอกสารโบราณ วิศวกรปัญญาประดิษฐ์และนักวิจัยข้อมูลระบบ สามารถประยุกต์ใช้งานสคริปต์ Python ที่มีความเร็วในการทำงานสูงด้านล่างนี้ ในการรันโมดูล Sauvola Adaptive Binarization เพื่อล้างคราบรบกวนบนหน้าใบลานเสื่อมสภาพ และสกัดลายเส้นอักขระขยายออกมาได้อย่างแม่นยำเต็มประสิทธิภาพ:
...
if __name__ == "__main__":
    ...
    historical_paper_sample = np.ones((150, 800), dtype=np.uint8) * 160
    
    # เพิ่มวงคราบหมึกเปื้อนสีดำจาง (รอยเปื้อนหมึกซึมผ่านข้ามหลัง)
    cv2.circle(historical_paper_sample, (400, 75), 60, 110, -1)
    
    # เขียนสัญญะจำลองลักษณะตัวอักษรเขียนหวัดอาจามีสีดำคมชัด (ค่าความเข้ม 20)
    # แสดงเสียงพยัญชนะดัดแปลงเฉพาะตัวของ Hausa
    cv2.putText(historical_paper_sample, "Ajami Hausa /ɗa/ 2025", (100, 95), 
    ...
    clean_result = binarizer.binarize(historical_paper_sample)
    
    # ประเมินสถิติและเปรียบเทียบผลลัพธ์
    black_pixel_ratio_raw = np.sum(historical_paper_sample < 100) / historical_paper_sample.size
    black_pixel_ratio_clean = np.sum(clean_result == 0) / clean_result.size
    
    print(f"มิติของภาพจำลองหน้าเอกสารอาจามี: {historical_paper_sample.shape}")
...
รายงานฉบับนี้นำเสนอการวิเคราะห์ระบบการจำแนกและการถอดอักษรจารึกอาจามี (Ajami) ซึ่งเป็นอักษรดัดแปลงจากภาษาอาหรับเพื่อบันทึกภาษาท้องถิ่นของแอฟริกาตะวันตก ได้แก่ ภาษาฟูลฟุลเด (Fulfulde) และภาษาฮอซา (Hausa) โดยใช้สถาปัตยกรรมโครงข่ายประสาทเทียมแบบผสมผสานระหว่างโมดูลการสกัดฟีเจอร์เชิงแสง (CNN) และโมดูลวิเคราะห์ลำดับเวลา (RNN/GRU) ควบคู่กับฟังก์ชัน Connectionist Temporal Classification (CTC Loss) โครงการนี้ได้พิสูจน์ถึงประสิทธิภาพการประมวลผลเอกสารประวัติศาสตร์ที่มีรอยชำรุดและการเขียนหวัดระดับสูง การวิจัยดังกล่าวส่งมอบแนวทางที่เป็นประโยชน์ต่อการฟื้นฟูระบบการบันทึกคัมภีร์ใบลานไทยโบราณที่ใช้อักษรพิเศษเช่นกัน

---

### เชิงอรรถ

[1] Usman Yousuf, Abubakar Aminu, et al., "A Handwritten Text Recognition Dataset for Ajami Manuscripts in Fulfulde and Hausa," in *Lecture Notes in Computer Science*, vol. 14243 (Cham: Springer, 2025), 415–430, https://doi.org/10.1007/978-3-032-04627-7_36.
